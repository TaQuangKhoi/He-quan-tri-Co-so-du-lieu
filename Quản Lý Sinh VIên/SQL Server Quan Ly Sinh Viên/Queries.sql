...
--f
SELECT MTRUOC.TENMH 'M�n tr??c', MSAU.TENMH 'M�n sau'
...
--g
SELECT MSAU.TENMH 'M�n sau', MTRUOC.TENMH 'M�n tr??c'
FROM MHOC MTRUOC, DKIEN D, MHOC MSAU
WHERE MTRUOC.MAMH = D.MAMH_TRUOC 
AND D.MAMH = MSAU.MAMH
...
SELECT H.MAHP, COUNT(S.MASV) 'S? l??ng sinh vi�n ??ng k�'
...
-- CACH NGAN GON ?AY ?U
SELECT K.MAHP, COUNT(K.MASV) 'S? l??ng sinh vi�n ??ng k�'
FROM KQUA K
GROUP BY K.MAHP

--i
SELECT S.MASV, S.TEN, --H.MAHP
	 H.NAM, SUM(K.DIEM * M.TINCHI) / SUM(M.TINCHI) '?i?m trung b�nh'
